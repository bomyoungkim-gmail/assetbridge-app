...
**Forward-run em produção (2026-06-18):** o grafo adotado aqui passou a ser o caminho de produção, não só infra testada — `/resolve-iup` e `/ingest` disparam o `graph.invoke` forward (1 `thread_id` por ativo/posição), ligando o contrato sync/pending inteiro (interrupt/resume/`/status`/`PostgresSaver`). Detalhe da decisão e do branch dual do HITL: ver revisão de 2026-06-18 no ADR-0010.
